...
getEst.grid <- function(param.space){
  ...
  pp.s <- vapply(pp,getSumLoglk,FUN.VALUE = numeric(2))
  pp.ss <- data.frame(mu1=pp,sumlikelihood=pp.s[1,])
  pp.n <- vapply(pn,getSumLoglk,FUN.VALUE = numeric(2))
  pp.nn <- data.frame(mu1=pn,sumlikelihood=pp.n[1,])

  if(sum(pp.ss$sumlikelihood)>sum(pp.nn$sumlikelihood)){
    ps <- pp.ss
  }else{
    ps <- pp.nn
  }
  ps$w <- exp(ps$sumlikelihood)/sum(exp(ps$sumlikelihood))
  mu1.mean = sum(ps$mu1*ps$w)
  pi <- getSumLoglk(mu1.mean)
  return(c(mu1.mean, pi[[2]]))
}
