...
'''
Note on next development we should use param as short form for parameter
########################################################################
...
def getPS_TMF_sigma(pmDef):
    sigma1Sp = list(np.linspace(pmDef['sigma1ul'],pmDef['sigma1uu'],pmDef['sigma1_sp_size']))+list(np.linspace(pmDef['sigma1ll'],pmDef['sigma1lu'],pmDef['mu1_sp_size']))
...
def Bsquid_sigma(theObject):
    ...
    for i in np.arange(runStr,len(y)):
        ...
        if i == runStr:
            y0 = y[0:runStr]
            pmSp_tmp = pd.DataFrame([getLikelihood_sigma(pi0,p,p21,sigma1,y0) for mu1 in pmSp['sigma1']],columns=['sigma1','l','piTheta'])
            pmSp['l'] = pmSp_tmp['l']
            pmSp['piTheta'] = pmSp_tmp['piTheta']
            pmSp['posd']=np.exp(pmSp['l']-pmSp['l'].mean(axis=0))/sum(np.exp(pmSp['l']-pmSp['l'].mean(axis=0)))
            opt.loc[i] = [i,sum(pmSp.mu1*pmSp.posd),sum(pmSp.piTheta*pmSp.posd),0,0]
...
def getLikelihood_sigma(pi0,p,p21,sigma,y):
    l0 = scipy.stats.norm(0,1).pdf(y)
    l1 = scipy.stats.norm(0,sigma).pdf(y)
    pi = []
    f = []
    for i in range(len(y)):
        if i == 0:
                ...
                f.append(np.dot(np.dot(p,pi[i-1]),[l0[i],l1[i]]))
                pi.append(np.dot(p,pi[i-1])*[l0[i],l1[i]]/f[i])
    return([sigma,sum(np.log(f)),pi[len(pi)-1][1]])
